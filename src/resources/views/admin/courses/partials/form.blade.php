@php
    ...
    $selectedSchoolId = old('school_id', $selectedSchoolId ?? $course->school_id ?? auth()->user()?->school_id);
    $selectedGradeId = old('grade_id', $selectedGradeId ?? $course->grade_id ?? '');
            ...
            <div class="row g-3">
                <div class="col-12 col-md-6">
                    ...
                    @else
                        ...
                        <div class="form-text">
                            Tu rol solo permite gestionar cursos del colegio asignado.
                        </div>
                    @endif
    ...
    <div class="col-12 col-xl-4">
        <div class="admin-card bg-white p-4 h-100">
            ...
            <div class="small text-secondary d-flex flex-column gap-2">
                ...
                <div>
                    <strong>Grado:</strong>
                    {{ $selectedGrade?->label ?: $selectedGrade?->name ?: 'Sin definir' }}
                </div>
                <div>
                    <strong>Curso:</strong>
                    {{ old('name', $course->name ?? 'Sin definir') }}
                </div>
                <div>
                    <strong>Etiqueta:</strong>
                    {{ old('label', $course->label ?? 'Sin definir') }}
                </div>
                <div>
                    <strong>Estado:</strong>
                    {{ old('is_active', $course->is_active ?? true) ? 'Activo' : 'Inactivo' }}
                </div>
            </div>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (schoolSelect?.value && gradeSelect.options.length <= 1) {
