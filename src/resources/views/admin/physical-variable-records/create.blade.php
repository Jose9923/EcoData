                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="observations" rows="4" class="form-control rounded-4"
                                      placeholder="Anotaciones opcionales del registro...">{{ old('observations') }}</textarea>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    async function loadGrades() {
        ...
        gradeSelect.innerHTML = optionHTML(data, 'Todos / Sin asignar');
    }
    ...
    async function loadCourses() {
        ...
        const data = await res.json();
        courseSelect.innerHTML = optionHTML(data, 'Todos / Sin asignar');
    }
        ...
        if (variable.data_type === 'integer' || variable.data_type === 'decimal') {
            ...
            input = `
                <select name="values[${variable.id}]" class="form-select rounded-4">
                    ...
                    <option value="1" ${String(oldValue) === '1' ? 'selected' : ''}>Sí</option>
                    <option value="0" ${String(oldValue) === '0' ? 'selected' : ''}>No</option>
                </select>
            `;
